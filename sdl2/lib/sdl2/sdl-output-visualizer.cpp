...
SDLOutputVisualizer::SDLOutputVisualizer(int x, int y) {
  window_ = std::unique_ptr<SDL_Window, SDLWindowDeleter>(
      SDL_CreateWindow(
          "lcSprinkler",
          x, y,
          128 * kDisplayScaleFactor, 16 * kDisplayScaleFactor,
...
void SDLOutputVisualizer::Refresh() {
  SDL_RenderPresent(renderer_.get());
}

void SDLOutputVisualizer::BindToWindow(SDL_Window *window) {
...
void SDLOutputVisualizer::Tick() {
  ...
  if (bound_window_ != nullptr) {
    ...
    SDL_GetWindowPosition(bound_window_,
                          &pos_x,
                          &pos_y);
    SDL_GL_GetDrawableSize(bound_window_,
                           &width,
                           &height);
    SDL_GetWindowBordersSize(bound_window_,
                             &top_border_width,
                             &left_border_width,
                             &bottom_border_width,
                             &right_border_width);
    SDL_SetWindowPosition(window_.get(),
                          pos_x,
                          pos_y + height + bottom_border_width);
  }
}
SDL_Window *SDLOutputVisualizer::GetWindow() {
  return window_.get();
}
